...
const TrailChart = (props) => {
    ...
    const [chartImage, setChartImage] = useState(<svg></svg>);

    useEffect(() => {
        function CandlestickChart(data, {
            date = d => d.visitDate, // given d in data, returns the (temporal) x-value
      ...
      High: ${formatValue(Yh[i])}`;
        ...
        setChartImage(CandlestickChart(data));
    }, []);

    return (
        <>
            {chartImage && (
                <svg dangerouslySetInnerHTML={{ __html: chartImage.innerHTML }} />
            )}
        </>
    );
};
